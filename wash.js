...
const web3 = new Web3('https://sgb-triforce.xyz:9650/ext/C/rpc');
...
const startBlock = process.argv[3] ? parseInt(process.argv[3]) : 30596167; // Default to 30596167 if not provided
...
const recurrenceThreshold = 10; // Set threshold
const timeThreshold = 24 * 60 * 60; // Time in seconds (one week)

async function initialize() {
	endBlock = process.argv[4] ? parseInt(process.argv[4]) : await web3.eth.getBlockNumber(); // Use provided end block or current block number
...
// Function to detect wash trading
function detectWashTrading(transactions) {
	...
	transactions.forEach(tx => {
		if (tx.from === excludedAddress || tx.to === excludedAddress) {
			return;
		}
		
		let pairKey = [tx.from, tx.to].sort().join('-');
		if (!tradingPairs.has(pairKey)) {
			tradingPairs.set(pairKey, { count: 0, transactions: [], timestamps: [] });
		}
	...
	tradingPairs.forEach((data, pairKey) => {
		if (data.count >= recurrenceThreshold && checkTimeThreshold(data.timestamps, timeThreshold)) {
...
function checkTimeThreshold(timestamps, threshold) {
	...
	for (let timestamp of timestamps) {
		if (timestamp - startTime <= threshold) {
			count++;
		} else {
			startTime = timestamp;
			count = 1;
...
async function fetchAndFilterTransactions(contractAddress, fromBlock, toBlock) {
  try {
    // Make an API call to the endpoint to fetch transactions
    const response = await web3.eth.getPastEvents('Transfer', {
      filter: { from: contractAddress },
      fromBlock: fromBlock,
      toBlock: toBlock
    });

    // Filter transactions for type 1 and value greater than 0
    const filteredTransactions = response.filter(tx => tx.transactionType === '1' && tx.value > 0);
    ...
    // Return or output the processed information
...
// Function to fetch and analyze transactions
async function fetchAndAnalyzeTransactions() {
	...
	try {
		...
		for (let event of events) {
			...
			// Check if the transaction has a value greater than zero
			if (transactionDetails.value > 0) {
				transactions.push({
					...
					value: web3.utils.fromWei(transactionDetails.value, 'ether') // Convert the value from Wei to Ether
				});
			}
		}
